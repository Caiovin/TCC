package br.com.sge.bean;

public class AlunoBean {
    private int rmAluno;
    private int codResponsavel;
    private String nmResponsavel;
    private String nmAluno;
    private String sexoAluno;
    private String rgAluno;
    private String cpfAluno;
    private String dtNascAluno;
    private String telAluno;
    private String celAluno;
    private String endAluno;
    private String emailAluno;
    private String nmTurma;
    private String prdAluno;
    private int codTurma;
    
    
	/**
	 * @return the sexoAluno
	 */
	public String getSexoAluno() {
		return sexoAluno;
	}
	/**
	 * @param sexoAluno the sexoAluno to set
	 */
	public void setSexoAluno(String sexoAluno) {
		this.sexoAluno = sexoAluno;
	}
	/**
	 * @return the rmAluno
	 */
	public int getRmAluno() {
		return rmAluno;
	}
	/**
	 * @param rmAluno the rmAluno to set
	 */
	public void setRmAluno(int rmAluno) {
		this.rmAluno = rmAluno;
	}
	/**
	 * @return the codResponsavel
	 */
	public int getCodResponsavel() {
		return codResponsavel;
	}
	/**
	 * @param codResponsavel the codResponsavel to set
	 */
	public void setCodResponsavel(int codResponsavel) {
		this.codResponsavel = codResponsavel;
	}
	/**
	 * @return the nmResponsavel
	 */
	public String getNmResponsavel() {
		return nmResponsavel;
	}
	/**
	 * @param nmResponsavel the nmResponsavel to set
	 */
	public void setNmResponsavel(String nmResponsavel) {
		this.nmResponsavel = nmResponsavel;
	}
	/**
	 * @return the nmAluno
	 */
	public String getNmAluno() {
		return nmAluno;
	}
	/**
	 * @param nmAluno the nmAluno to set
	 */
	public void setNmAluno(String nmAluno) {
		this.nmAluno = nmAluno;
	}
	/**
	 * @return the rgAluno
	 */
	public String getRgAluno() {
		return rgAluno;
	}
	/**
	 * @param rgAluno the rgAluno to set
	 */
	public void setRgAluno(String rgAluno) {
		this.rgAluno = rgAluno;
	}
	/**
	 * @return the cpfAluno
	 */
	public String getCpfAluno() {
		return cpfAluno;
	}
	/**
	 * @param cpfAluno the cpfAluno to set
	 */
	public void setCpfAluno(String cpfAluno) {
		this.cpfAluno = cpfAluno;
	}
	/**
	 * @return the dtNascAluno
	 */
	public String getDtNascAluno() {
		return dtNascAluno;
	}
	/**
	 * @param dtNascAluno the dtNascAluno to set
	 */
	public void setDtNascAluno(String dtNascAluno) {
		this.dtNascAluno = dtNascAluno;
	}
	/**
	 * @return the telAluno
	 */
	public String getTelAluno() {
		return telAluno;
	}
	/**
	 * @param telAluno the telAluno to set
	 */
	public void setTelAluno(String telAluno) {
		this.telAluno = telAluno;
	}
	/**
	 * @return the celAluno
	 */
	public String getCelAluno() {
		return celAluno;
	}
	/**
	 * @param celAluno the celAluno to set
	 */
	public void setCelAluno(String celAluno) {
		this.celAluno = celAluno;
	}
	/**
	 * @return the endAluno
	 */
	public String getEndAluno() {
		return endAluno;
	}
	/**
	 * @param endAluno the endAluno to set
	 */
	public void setEndAluno(String endAluno) {
		this.endAluno = endAluno;
	}
	/**
	 * @return the emailAluno
	 */
	public String getEmailAluno() {
		return emailAluno;
	}
	/**
	 * @param emailAluno the emailAluno to set
	 */
	public void setEmailAluno(String emailAluno) {
		this.emailAluno = emailAluno;
	}
	/**
	 * @return the nmTurma
	 */
	public String getNmTurma() {
		return nmTurma;
	}
	/**
	 * @param nmTurma the nmTurma to set
	 */
	public void setNmTurma(String nmTurma) {
		this.nmTurma = nmTurma;
	}
	/**
	 * @return the prdAluno
	 */
	public String getPrdAluno() {
		return prdAluno;
	}
	/**
	 * @param prdAluno the prdAluno to set
	 */
	public void setPrdAluno(String prdAluno) {
		this.prdAluno = prdAluno;
	}
	/**
	 * @return the codTurma
	 */
	public int getCodTurma() {
		return codTurma;
	}
	/**
	 * @param codTurma the codTurma to set
	 */
	public void setCodTurma(int codTurma) {
		this.codTurma = codTurma;
	}
    
       
}